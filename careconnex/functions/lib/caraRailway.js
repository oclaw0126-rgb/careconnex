...
exports.disconnectCara = exports.getCaraHistory = exports.caraWebhook = exports.connectCaraAgent = void 0;
 ...
 */
exports.connectCaraAgent = functions.https.onRequest(async (req, res) => {
    var _a, _b, _c, _d;
    ...
    res.set(corsHeaders);
    try {
        const { name, phone, email, telegramUsername, messagingPreference = 'whatsapp', zipCode, needs, schedule } = req.body;
        if (!phone) {
        ...
        const seniorRef = await db.collection('seniors').add({
            ...
            name: (name === null || name === void 0 ? void 0 : name.split(' ')[0]) || 'Senior',
            careNeeds: needs || [],
            zipCode: zipCode || null,
            active: true,
            createdAt: admin.firestore.FieldValue.serverTimestamp()
        ...
        const twilioWhatsApp = (_c = functions.config().twilio) === null || _c === void 0 ? void 0 : _c.whatsapp_number;
        if (twilioSid && twilioToken) {
            ...
            if (messagingPreference === 'whatsapp') {
                ...
                await twilio.messages.create({
                    body: welcomeMessage,
                    from: (_d = functions.config().twilio) === null || _d === void 0 ? void 0 : _d.phone_number,
                    to: phone
                });
            }
 ...
 */
exports.caraWebhook = functions.https.onRequest(async (req, res) => {
    if (req.method === 'OPTIONS') {
        res.set(corsHeaders);
        res.status(204).send('');
        return;
    ...
    res.set(corsHeaders);
    try {
        const { phoneNumber, message, response, toolCalls, familyId, seniorId } = req.body;
 ...
 */
exports.getCaraHistory = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
    ...
    const phone = family === null || family === void 0 ? void 0 : family.phone;
    if (!phone) {
        return { messages: [] };
    }
    ...
    const messagesSnapshot = await db.collection('cara_conversations')
        ...
        .limit(50)
        .get();
    const messages = messagesSnapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data()))).reverse();
    return { messages };
});
/**
 * Disconnect Cara for a family
 */
exports.disconnectCara = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Must be logged in');
    }
    const { familyId } = data;
...
//# sourceMappingURL=caraRailway.js.map
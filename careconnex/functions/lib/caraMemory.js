...
exports.consolidateMemories = exports.syncFamilyMemories = void 0;
...
exports.generateMemorySummary = generateMemorySummary;
 ...
 */
async function storeMemory(userId, category, key, value, importance = 'medium') {
 ...
 */
async function getMemories(userId, category, limit = 20) {
    let query = db.collection('cara_memories')
        .where('userId', '==', userId)
...
async function searchMemories(userId, searchTerm) {
    ...
    return memories.filter(m => {
        var _a, _b, _c;
        return ((_a = m.key) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes(term)) ||
            ((_b = JSON.stringify(m.value)) === null || _b === void 0 ? void 0 : _b.toLowerCase().includes(term)) ||
            ((_c = m.category) === null || _c === void 0 ? void 0 : _c.toLowerCase().includes(term));
    });
}
/**
 ...
 */
async function extractAndStoreMemories(userId, userMessage, assistantMessage) {
    var _a;
    ...
    for (const condition of conditions) {
        if (message.includes(condition)) {
            ...
            const hasCondition = existingConditions.some(m => { var _a; return m.key === 'conditions' && ((_a = m.value) === null || _a === void 0 ? void 0 : _a.includes(condition)); });
            if (!hasCondition) {
                const currentConditions = ((_a = existingConditions.find(m => m.key === 'conditions')) === null || _a === void 0 ? void 0 : _a.value) || [];
 ...
 */
exports.syncFamilyMemories = functions.firestore
    ...
    for (const member of familyMembers.docs) {
        if (member.id !== after.userId) {
            ...
            batch.set(syncRef, Object.assign(Object.assign({}, after), { userId: member.id, syncedFrom: after.userId, isSynced: true, syncedAt: admin.firestore.FieldValue.serverTimestamp() }));
        }
    }
    await batch.commit();
});
 ...
 */
async function generateMemorySummary(userId) {
...
//# sourceMappingURL=caraMemory.js.map
 ...
 * 
 * When a caregiver cancels or calls out:
 * 1. Notify client immediately
 * 2. Find 3 alternative caregivers who match the requirements
 * 3. Send options to client via push, SMS, and email
 * 4. Allow client to select new caregiver or request refund
 */
...
async function createNotification(userId: string, notification: {
    title: string;
    body: string;
    type: 'booking' | 'message' | 'system' | 'alert' | 'callout';
 ...
 */
async function sendEmailToUser(userId: string, subject: string, htmlContent: string, textContent: string) {
    if (!resend) {
 ...
 */
async function findBackupCaregivers(appointment: Appointment, originalCaregiverId: string): Promise<CaregiverProfile[]> {
    try {
        ...
        for (const doc of caregiverDocs) {
            ...
            const hasRequiredSkills = serviceType ? 
                data.skills?.some(skill => serviceType.toLowerCase().includes(skill.toLowerCase())) : 
                true;
 ...
 */
function checkAvailability(availability: any, date: string, time: string): boolean {
    if (!availability) return true; // Assume available if no availability set
    ...
    const startHour = parseInt(dayAvailability.startTime?.split(':')[0] || '0');
    const endHour = parseInt(dayAvailability.endTime?.split(':')[0] || '23');
 ...
 */
function generateCalloutEmailHtml(caregiverName: string, date: string, time: string, backupCaregivers: CaregiverProfile[]): string {
        ...
        <div style="border: 1px solid #e2e8f0; border-radius: 12px; padding: 16px; margin: 12px 0; background: white;">
            <div style="display: flex; align-items: center; gap: 12px;">
                ...
                <div style="flex: 1;">
                    ...
                    <div style="color: #64748b; font-size: 14px;">⭐ ${c.rating?.toFixed(1) || '4.5'} • $${c.hourlyRate}/hr</div>
                </div>
            </div>
 ...
 */
function generateCalloutEmailText(caregiverName: string, date: string, time: string, backupCaregivers: CaregiverProfile[]): string {
    const caregiverList = backupCaregivers.map((c, i) => 
        `${i + 1}. ${c.firstName} ${c.lastName}${i === 0 ? ' (Best Match)' : ''}
   Rating: ${c.rating?.toFixed(1) || '4.5'} | Rate: $${c.hourlyRate}/hr
   Skills: ${c.skills?.slice(0, 3).join(', ') || 'General Care'}`
    ).join('\n\n');
 ...
 */
function calculateMatchScore(caregiver: CaregiverProfile, appointment: Appointment): number {
    let score = 0;
    ...
    if (appointment.serviceType && caregiver.skills?.some(s => 
        appointment.serviceType.toLowerCase().includes(s.toLowerCase()))) {
        score += 5;
    }
    
    // Competitive rate (3 points if under $30/hr)
    if (caregiver.hourlyRate < 30) score += 3;
 ...
 */
async function notifyClientOfCallout(appointment: Appointment, backupCaregivers: CaregiverProfile[]) {
    ...
    try {
        ...
        const smsMessage = SMS_TEMPLATES.caregiverCallout(
            caregiverName,
            date,
            time,
            backupCaregivers.length,
            backupNames
        );
        await sendSMSToUser(clientId, smsMessage);
 ...
 */
export const selectBackupCaregiver = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        await sendSMSToUser(
            backupCaregiverId,
            SMS_TEMPLATES.backupCaregiverAssigned(
 ...
 */
export const requestCalloutRefund = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
 ...
 */
export const getBackupCaregiverOptions = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }

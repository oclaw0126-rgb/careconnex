...
export class CaraAgent {
  ...
  constructor(userId: string | null, userPhone: string, conversationId: string, userName: string = '') {
    this.userId = userId;
  ...
  async loadUserMemory() {
    ...
    try {
      ...
      if (userDoc.exists) {
        ...
        this.userName = data.name?.split(' ')[0] || this.userName;
      }
    } catch (error) {
  ...
  async answerWithPersonality(message: string): Promise<any> {
  ...
  checkForEmergency(message: string): { isEmergency: boolean; type?: string } {
  ...
  async toolHandleEmergency(emergency: any): Promise<any> {
  ...
  async toolQueryCaregivers(params: any): Promise<any> {
    ...
    try {
      ...
      snapshot.forEach((doc: any) => {
        ...
        if (careNeeds.length > 0 && cg.skills) {
          const matchingSkills = careNeeds.filter((need: string) => 
            cg.skills.some((skill: string) => skill.toLowerCase().includes(need.toLowerCase()))
          );
        ...
        if (cg.zipCode === userZip) score += 15;
        if (budget && cg.hourlyRate <= budget) score += 10;
        
        caregivers.push({ id: doc.id, ...cg, matchScore: score });
      });
  ...
  async toolCreateBooking(params: any): Promise<any> {
    ...
    try {
      ...
      if (caregiverIndex !== undefined && this.context.lastCaregiverSearch) {
        targetCaregiverId = this.context.lastCaregiverSearch[caregiverIndex]?.id;
      }
  ...
  async notifyCaregiverOfBooking(caregiver: any, bookingId: string, schedule: string[], startDate: string): Promise<void> {
    try {
  ...
  async toolCheckAppointments(): Promise<any> {
    if (!this.userId) {
      return {
    ...
    try {
      ...
      snapshot.forEach((doc: any) => appointments.push({ id: doc.id, ...doc.data() }));
  ...
  async toolCancelAppointment(params: any): Promise<any> {
  ...
  async notifyCaregiverOfCancellation(appointment: any): Promise<void> {
    // Implementation
  }

  async toolScheduleInterview(params: any): Promise<any> {
  ...
  async toolFindBackupCaregiver(params: any): Promise<any> {
    return {
  ...
  async toolSetMedicationReminder(params: any): Promise<any> {
    return {
  ...
  async toolWellnessCheck(params: any): Promise<any> {
  ...
  async toolSendFamilyUpdate(params: any): Promise<any> {
  ...
  async toolRequestCaregiverFeedback(params: any): Promise<any> {
    return {
  ...
  async toolSubmitFeedback(params: any): Promise<any> {
  ...
  async updateCaregiverRating(caregiverId: string, newRating: number): Promise<void> {
  ...
  async toolCheckWeather(params: any): Promise<any> {
    ...
    try {
      ...
      const appointments: any[] = [];
      appointmentsSnapshot.forEach((doc: any) => appointments.push(doc.data()));
      ...
      const rainDays = appointments.filter((appt: any) => {
...
\n\nWould you like me to:\n• Enable automatic weather alerts?\n• Reschedule if severe weather forecasted?`,
  ...
  async toolGetEmergencyContact(): Promise<any> {
  ...
  async toolHandleDocumentUpload(params: any): Promise<any> {
  ...
  async toolViewDocuments(): Promise<any> {
    try {
      ...
      const documents: any[] = [];
      snapshot.forEach((doc: any) => documents.push({ id: doc.id, ...doc.data() }));
      ...
      return {
        text: `📁 **Your Documents (${documents.length})**\n\n${documents.slice(0, 5).map((doc: any, i: number) => 
          `${i+1}. **${doc.description || doc.type}**\n   📅 ${doc.uploadedAt?.toDate().toLocaleDateString() || 'Recent'}\n   ${doc.sharedWithCaregivers ? '👥 Shared with caregiver' : '🔒 Private'}`
        ).join('\n\n')}\n\nReply with document number to view, share, or download.`,
        data: { documents },
        actions: documents.slice(0, 5).map((doc: any, i: number) => ({
          type: 'view_document',
  ...
  async toolAnswerBilling(params: any): Promise<any> {
    ...
    try {
      ...
      const invoices: any[] = [];
      invoicesSnapshot.forEach((doc: any) => invoices.push({ id: doc.id, ...doc.data() }));
      ...
      const paymentMethod = userData?.defaultPaymentMethod || { brand: 'Visa', last4: '4242' };
      
      return {
        text: `💰 **Your Billing Summary**\n\n**Current Balance:** $${totalDue.toFixed(2)}\n\n**Recent Invoices:**\n${invoices.slice(0, 3).map((inv: any) => 
          `• ${inv.date || 'Recent'}: $${inv.amount} - ${inv.caregiverName || 'Care'} (${inv.status})`
        ).join('\n')}\n\n**Payment Method:** ${paymentMethod.brand} ending in ${paymentMethod.last4}\n**Next Due:** ${unpaidInvoices[0]?.dueDate || 'None'}\n\nWhat would you like to do?`,
        actions: [
  ...
  async toolCallCaregiver(params: any): Promise<any> {
  ...
  parseIntent(message: string): any {
    const text = message.toLowerCase();
    ...
    if (text.includes('find') || text.includes('search')) return { type: 'QUERY_CAREGIVERS', params: {} };
    if (text.includes('my appointments') || text.includes('schedule')) return { type: 'CHECK_APPOINTMENT', params: {} };
    if (text.includes('cancel')) return { type: 'CANCEL_APPOINTMENT', params: {} };
    if (text.includes('interview')) return { type: 'SCHEDULE_INTERVIEW', params: {} };
    if (text.includes('medication') || text.includes('pill')) return { type: 'MEDICATION_REMINDER', params: {} };
    if (text.includes('how is') || text.includes('wellness')) return { type: 'WELLNESS_CHECK', params: {} };
    if (text.includes('family') || text.includes('update')) return { type: 'FAMILY_UPDATE', params: {} };
    if (text.includes('feedback') || text.includes('rate')) return { type: 'CAREGIVER_FEEDBACK', params: {} };
    if (text.includes('weather')) return { type: 'WEATHER_CHECK', params: {} };
    if (text.includes('emergency')) return { type: 'EMERGENCY_CONTACT', params: {} };
    if (text.includes('document') || text.includes('upload')) return { type: 'DOCUMENT_HELP', params: {} };
    if (text.includes('my documents') || text.includes('view doc')) return { type: 'DOCUMENT_VIEW', params: {} };
    if (text.includes('bill') || text.includes('payment')) return { type: 'BILLING_QUESTION', params: {} };
  ...
  async logInteraction(role: string, message: string, metadata: any = {}) {
    try {
      if (this.userId) {

...
// Cara Agent LLM - Using REST API instead of SDK for reliability
...
Respond in this JSON format:
{
  ...
  const history = conversationHistory.slice(-5).map(h => 
    `${h.role === 'user' ? 'User' : 'Cara'}: ${h.content}`
  ).join('\n');
 ...
 */
export function extractInfo(message: string): Record<string, any> {

...
import { processWithLLM, UserData, ConversationMessage } from './caraAgentLLM';
import { tools } from './caraTools';
...
export const whatsappWebhook = functions.https.onRequest(async (req, res) => {
  try {
    ...
    const conversationHistory: ConversationMessage[] = historySnapshot.docs
      .reverse()
      .map(doc => ({
 ...
 */
async function handleNewUser(profileName: string | undefined): Promise<{ message: string; tool?: null; contextUpdate?: null }> {
    ...
    setTimeout(async () => {
      try {
        ...
        if (searchResult.found && searchResult.caregivers && searchResult.caregivers.length > 0) {
          const caregiverList = searchResult.caregivers.map((c: any, i: number) =>
 ...
 */
export const whatsappHealth = functions.https.onRequest((req, res) => {
  res.json({
    status: 'healthy',
 ...
 */
async function handleFallbackResponse(message: string, userData: UserData): Promise<{ message: string; tool?: string | null; contextUpdate?: { key: string; value: any } | null }> {
  const text = message.toLowerCase();
  console.log('🔧 Fallback processing:', text);

  // Extract zip code
  const zipMatch = message.match(/(\d{5})/);
  const zipCode = zipMatch ? zipMatch[1] : userData.context?.zipCode;
  ...
  if (text.includes('find') || text.includes('search') || text.includes('looking for') ||
    ...
    if (zipCode) {
      ...
      try {
        const result = await tools.searchCaregivers({
          zipCode,
        ...
        if (result.found && result.caregivers && result.caregivers.length > 0) {
          const caregiverList = result.caregivers.map((c: any, i: number) =>
            `${i + 1}️⃣ *${c.name}* - $${c.hourlyRate}/hr, ⭐ ${c.rating}\n   ${c.specialties?.slice(0, 2).join(', ')}`
          ).join('\n\n');

...
/**
 * ============================================================
 * CARA AGENT CORE - OpenClaw-Inspired Architecture
 * ============================================================
 * 
 * Design Principles from OpenClaw:
 * 1. Tools have schemas - LLM knows exactly what each tool does
 * 2. Agent Loop: Think → Choose Tool → Execute → Respond
 * 3. Memory is persistent and searchable
 * 4. Skills are modular and composable
 */
  ...
  search_caregivers: {
    ...
    execute: async (params, context) => {
      ...
      try {
        ...
        let caregivers = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        
        if (params.care_needs?.length > 0) {
          caregivers = caregivers.map(c => ({
            ...c,
            match_score: calculateMatchScore(c, params.care_needs)
          })).sort((a, b) => b.match_score - a.match_score);
        }

        const topMatches = caregivers.slice(0, 3).map((c: any, i: number) => ({
          rank: i + 1,
          id: c.id,
          name: c.name,
          ...
          bio: c.bio?.substring(0, 100) + '...' || 'Experienced caregiver',
...
function calculateMatchScore(caregiver: any, needs: string[]): number {
  ...
  const matches = needs.filter(need =>
    allSkills.some((skill: string) => skill.includes(need.toLowerCase()))
  ).length;
...
export async function processMessage(
  ...
  const toolsSchema = Object.values(TOOLS).map(t => ({
    name: t.name,

 ...
 */
export function generateGoogleCalendarLink(eventParams: {
  title: string;
  description: string;
  location?: string;
  startTime: Date;
...
): Promise<{ googleLink: string; icsContent: string }> {
 ...
 */
export async function createCareAppointmentCalendarEvent(
  userId: string,
 ...
 */
function parseTimeSlotToDate(dateStr: string, timeSlot: string): Date {
 ...
 */
export const onInterviewScheduledCalendar = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    const { googleLink, icsContent } = await createInterviewCalendarEvent(
      interview.userId,
      interviewId,
 ...
 */
export const onAppointmentConfirmedCalendar = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    if (before.status !== 'confirmed' && after.status === 'confirmed') {
      ...
      const { googleLink, icsContent } = await createCareAppointmentCalendarEvent(
        after.userId,
        appointmentId,
        after.caregiverName,
        after.schedule.date,
        after.schedule.timeSlot,
        after.schedule.duration || 4,
        after.schedule.location
      );
 ...
 */
export const weeklyScheduleSummary = functions.pubsub
  ...
  .onRun(async (context) => {
    ...
    for (const [userId, userAppointments] of Object.entries(byUser)) {
      ...
      if (!userData?.phoneNumber) continue;
      ...
      const scheduleList = userAppointments.map(a => 
        `• ${a.schedule.date}: ${a.caregiverName} (${a.schedule.timeSlot})`
      ).join('\n');
 ...
 */
export async function getUpcomingSchedule(userId: string, days: number = 7): Promise<any[]> {
...
export async function checkScheduleConflict(
  ...
  return {
    ...
    conflictingAppointments: conflicts.map(d => ({ id: d.id, ...d.data() }))
  };
}
